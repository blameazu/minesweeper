...
import {
    ...
    remainingMines,
    createEmptyState,
    reveal as replayReveal,
    toggleFlag as replayToggleFlag,
    chordReveal as replayChordReveal,
} from './lib/engine';
...
function App() {
    const { board, setDifficulty, startFresh, revealCell, toggleFlag, chordCell } =
        useGameStore();
    const [mode, setMode] = useState<'solo' | 'versus'>(() =>
        readStored(UI_MODE_KEY, 'solo', ['solo', 'versus'])
    );
    const [view, setView] = useState<'solo' | 'versus' | 'profile' | 'rank' | 'blog'>(
        () =>
            readStored(UI_VIEW_KEY, 'solo', [
                'solo',
                'versus',
                'profile',
                'rank',
    ...
    const [theme, setTheme] = useState<(typeof THEME_OPTIONS)[number]>(() =>
        readStored(UI_THEME_KEY, 'light', THEME_OPTIONS)
    );
    ...
    const [selectedResultPlayerId, setSelectedResultPlayerId] = useState<number | null>(
        null
    );
    ...
    const [soloDifficulty, setSoloDifficulty] = useState<DifficultyKey>(
        board.difficulty
    );
    ...
    const [lbReplayMeta, setLbReplayMeta] = useState<{
        player: string;
        timeMs: number;
        difficulty: DifficultyKey;
    } | null>(null);
    const [spectatorViewPlayerId, setSpectatorViewPlayerId] = useState<number | null>(
        null
    );
        ...
        try {
            ...
            setVsMatch({
                ...
                board: {
                    ...
                    mines: board.mines,
                    seed: board.seed,
                    safeStart: board.safeStart ?? null,
                },
                status: 'pending',
            });
                        ...
                        m
                            ? {
                                  ...
                                  board: {
                                      ...
                                      mines: state.mines,
                                      seed: state.seed,
                                      safeStart: state.safe_start ?? null,
                                  },
    ...
    useEffect(() => {
        const shouldTick =
            ...
            (mode === 'versus' && !isSpectator && vsState?.status === 'active');
    ...
    const recordSoloStep = useCallback(
        (action: ReplayStep['action'], x: number, y: number) => {
            if (mode !== 'solo') return;
            setSoloReplaySteps((prev) => [
                ...prev,
                { action, x, y, elapsed_ms: elapsedMs },
            ]);
        },
        [elapsedMs, mode]
    );
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                if (!cancelled)
                    setProfileError(
                        e instanceof Error ? e.message : '讀取個人資料失敗'
                    );
            } finally {
        ...
        const resume = async () => {
            try {
                ...
                if (
                    ...
                    return;
                const safeStart =
                    active.board.safe_start ?? active.board.safeStart ?? null;
                const boardConfig = {
                    ...
                    difficulty:
                        (active.board.difficulty as DifficultyKey | null) ?? null,
    ...
    const statusText = useMemo(() => {
        ...
        if (boardForView.status === 'lost')
            return mode === 'versus' ? '你輸了' : '踩到雷 QQ';
        return '進行中';
    }, [boardForView.status, mode]);

    const computeReplayDelay = useCallback(
        <T extends { elapsed_ms?: number | null }>(
            stepsArr: T[],
            idx: number,
            speedFactor: number
        ) => {
    ...
    const rebuildReplayBoard = useCallback(
        (
            ...
            stepsArr: Array<ReplayStep | MatchStep>,
            targetIndex: number
        ) => {
            if (!base) return null;
    ...
    useEffect(() => {
        ...
        useGameStore.setState((state) => {
            ...
            const status =
                state.board.status === 'idle' ? 'playing' : state.board.status;
    ...
    useEffect(() => {
        ...
        const loadRank = async () => {
            try {
                ...
                if (!cancelled)
                    setRankError(e instanceof Error ? e.message : '讀取排行失敗');
            } finally {
    ...
    useEffect(() => {
        if (vsMatch && !isSpectator) {
            localStorage.setItem(
                VS_SESSION_KEY,
                JSON.stringify({
                    matchId: vsMatch.matchId,
                    playerId: vsMatch.playerId,
                    ...
                    m
                        ? {
                              ...
                              board: {
                                  ...
                                  seed: state.seed,
                                  safeStart:
                                      state.safe_start ?? m.board.safeStart ?? null,
                              },
                              hostId: state.host_id ?? m.hostId ?? null,
                          }
                        : m
        ...
        useGameStore.setState((state) => {
            ...
            return {
                board: {
                    ...board,
                    status,
                    endedAt: board.endedAt ?? Date.now(),
                },
            };
        });
    ...
    useEffect(() => {
        ...
        setReplaySteps([]);
        const playerSnap =
            vsState?.players.find((p) => p.id === selectedResultPlayerId)?.progress ??
            null;
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            setLbReplayBoard((prev) =>
                prev ? applyReplayStep(prev, step as any) : prev
            );
    ...
    const applyBoardConfig = (config: {
        ...
        const safeStart =
            config.safe_start ?? config.safeStart ?? board.safeStart ?? null;
        setDifficulty(diff, {
            width: config.width,
    ...
    const handleCreateMatch = async () => {
        ...
        try {
            ...
            applyBoardConfig({
                ...
                mines: session.board.mines,
                seed: session.board.seed,
                difficulty: session.board.difficulty as DifficultyKey | null,
                safe_start: session.board.safeStart ?? null,
            });
    ...
    const handleLeaveMatch = async () => {
        ...
        try {
            ...
            if (!isSpectator && notStarted) {
                ...
                if (soloMatch) {
                    await deleteMatch(vsMatch.matchId, {
                        playerToken: vsMatch.playerToken,
                    });
                    setVsInfo('已退出並刪除對局');
                } else {
                    await leaveMatch(vsMatch.matchId, {
                        playerToken: vsMatch.playerToken,
                    });
                    setVsInfo('已退出對局');
                }
    ...
    const startLeaderboardReplay = async (entryOrId: LeaderboardEntry | number) => {
        const entryId =
            typeof entryOrId === 'number' ? entryOrId : Number(entryOrId.id);
        const fallbackPlayer = typeof entryOrId === 'number' ? null : entryOrId.player;
        const fallbackDiff =
            typeof entryOrId === 'number' ? null : entryOrId.difficulty;
        ...
        try {
            ...
            const diff =
                (data.difficulty as DifficultyKey | null) ?? fallbackDiff ?? 'beginner';
            ...
            setLbReplayIndex(0);
            setLbReplayMeta({
                player: data.player ?? fallbackPlayer ?? '',
                timeMs: data.time_ms ?? fallbackTime ?? 0,
                difficulty: diff as DifficultyKey,
            });
    ...
    const scrubLbReplay = useCallback(
        (targetIndex: number) => {
            const next = rebuildReplayBoard(
                lbReplayBase,
                lbReplaySteps as any[],
                targetIndex
            );
    ...
    const handleSetReady = async () => {
        ...
        try {
            ...
            await setReady(vsMatch.matchId, {
                playerToken: vsMatch.playerToken,
                ready: nextReady,
            });
        } catch (e) {
    ...
    const handleStartMatch = async () => {
        ...
        try {
            setVsError(null);
            setVsInfo('即將開始，請準備...');
            const res = await startMatch(vsMatch.matchId, {
                playerToken: vsMatch.playerToken,
            });
            setVsState((prev) =>
                prev
    ...
    const buildReplayBoard = () => {
        ...
        const baseDifficulty =
            (vsState.difficulty as DifficultyKey | null) ?? versusDifficulty;
    ...
    const loadSpectatorBoard = useCallback(
        async (targetPlayerId?: number) => {
            ...
            if (!player) return;
            const baseDifficulty =
                (currentState.difficulty as DifficultyKey | null) ?? versusDifficulty;
            ...
            try {
                ...
                if (player.progress?.board) {
                    ...
                    nextBoard = filtered.reduce(
                        (state, step) => applyReplayStep(state, step),
                        baseBoard
                    );
                }

                const startedAtMs =
                    parseUtcMillis(currentState.started_at) ??
                    nextBoard.startedAt ??
                    null;
                const finishedMs =
                    parseUtcMillis(player.finished_at as string | undefined) ??
                    parseUtcMillis(currentState.ended_at) ??
                    nextBoard.endedAt ??
                    null;
                let status = nextBoard.status;
                if (player.result === 'win') status = 'won';
                else if (player.result === 'lose' || player.result === 'forfeit')
                    status = 'lost';
                ...
                setSpectatorBoardError(
                    e instanceof Error ? e.message : '載入觀戰棋盤失敗'
                );
                setSpectatorBoard(null);
            } finally {
                setSpectatorBoardLoading(false);
        ...
        [
            isSpectator,
            spectatorBoardLoading,
            spectatorViewPlayerId,
            vsMatch?.matchId,
            versusDifficulty,
        ]
    );
    ...
    const startReplayForSelected = async () => {
        if (!vsMatch || !vsState) return;
        const player =
            vsState.players.find((p) => p.id === selectedResultPlayerId) ??
            vsState.players[0];
        if (!player) return;
    ...
    const handleSaveEditBlog = async () => {
        ...
        try {
            setBlogDetailError(null);
            const updated = await updateBlogPost(blogDetail.id, {
                title: blogEditTitle,
                content: blogEditContent,
                token,
            });
            setBlogDetail((prev) =>
    ...
    const handleAddBlogComment = async () => {
        ...
        try {
            setBlogDetailError(null);
            const comment = await addBlogComment(blogDetail.id, {
                content: blogComment,
                token,
            });
            setBlogDetail((prev) =>
    ...
    const handleVoteBlogPost = async (
        postId: number,
        value: -1 | 0 | 1,
        current?: number | null
    ) => {
        ...
        try {
            const updated = await voteBlogPost(postId, {
                value: nextValue,
                token,
            });
            setBlogPosts((prev) =>
                        ...
                        <div>
                            ...
                            {lbReplayMeta && (
                                <p className="text-sm opacity-80">
                                    玩家 {lbReplayMeta.player} · 難度{' '}
                                    {lbReplayMeta.difficulty} ·{' '}
                                    {formatMs(lbReplayMeta.timeMs)} 秒
                                </p>
                            )}
                        </div>
                                ...
                                <div className="flex flex-wrap items-center gap-3 justify-between">
                                    <div className="text-sm opacity-80">
                                        ...
                                        <span className="ml-3 text-xs">
                                            步驟 {Math.min(lbReplayIndex, totalSteps)} /{' '}
                                            {totalSteps}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-3 text-sm flex-wrap">
                                        ...
                                        <label className="flex items-center gap-2 text-xs opacity-80">
                                            ...
                                            <span className="tabular-nums text-[11px]">
                                                {Math.min(lbReplayIndex, totalSteps)} /{' '}
                                                {totalSteps}
                                            </span>
                                        </label>
                                    </div>
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                toggleTheme(
                                    e.target.value as (typeof THEME_OPTIONS)[number]
                                )
                            }
                ...
                <section className="space-y-4">
                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow p-4 space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-sm opacity-80">
                                {currentUser ? currentUser.handle : '請登入'}
                            </div>
                        </div>
                        {!isAuthenticated ? (
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <div className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 flex items-center justify-between">
                                            ...
                                            <span className="font-semibold">
                                                {profile.rank_counts.first}
                                            </span>
                                        </div>
                                        <div className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 flex items-center justify-between">
                                            <span>#2</span>
                                            <span className="font-semibold">
                                                {profile.rank_counts.second}
                                            </span>
                                        </div>
                                        <div className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 flex items-center justify-between">
                                            <span>#3</span>
                                            <span className="font-semibold">
                                                {profile.rank_counts.third}
                                            </span>
                                        </div>
                                        <div className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 flex items-center justify-between">
                                            <span>最後一名</span>
                                            <span className="font-semibold">
                                                {profile.rank_counts.last}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    {profile.match_history.length === 0 ? (
                                        ...
                                        <div className="space-y-2 text-sm">
                                            {profile.match_history.map((m) => (
                                                <div
                                                    key={m.match_id}
                                                    className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2"
                                                >
                                                    <div className="flex items-center justify-between">
                                                        <span className="font-semibold">
                                                            #{m.match_id}
                    ...
                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow p-4 space-y-3">
                        ...
                        {rankBoard?.me && (
                            <div className="mt-3 rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 text-sm flex items-center justify-between bg-gray-400">
                                ...
                                <span className="opacity-70">{rankBoard.me.score}</span>
                            </div>
                        )}
                    </div>
                </section>
                ...
                <section className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 text-xs">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        reloadBlogPosts(
                                            e.target.value as 'created' | 'score'
                                        )
                                    }
                        ...
                        {blogLoading ? (
                            ...
                            <div className="space-y-2">
                                {blogPosts.map((p) => (
                                    <div
                                        key={p.id}
                                        className="rounded border border-[var(--border)] bg-[var(--surface-strong)] p-3 space-y-2"
                                    >
                    ...
                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow p-4 space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            {blogDetailLoading && (
                                <span className="text-xs opacity-70">載入中...</span>
                            )}
                        </div>
                        {blogDetailError && (
                            <p className="text-sm text-red-600">{blogDetailError}</p>
                        )}
                        {!blogDetail ? (
                            <p className="text-sm opacity-70">
                                點擊上方「查看留言」以閱讀完整文章
                                ...
                                {blogEditing ? (
                                    <div className="space-y-2">
                                        <input
                                            value={blogEditTitle}
                                            onChange={(e) =>
                                                setBlogEditTitle(e.target.value)
                                            }
                                            maxLength={200}
                                            className="w-full rounded border border-[var(--border)] px-3 py-2 bg-[var(--surface-strong)]"
                                        />
                                        <textarea
                                            ...
                                            onChange={(e) =>
                                                setBlogEditContent(e.target.value)
                                            }
                                            maxLength={5000}
                                    ...
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: renderMarkdown(blogDetail.content),
                                        }}
                                    />
                                )}
                                <div className="space-y-2">
                                    <div className="font-semibold">
                                        留言 ({blogDetail.comment_count})
                                    </div>
                                    {blogDetail.comments.length === 0 ? (
                                        ...
                                        <div className="space-y-2">
                                            {blogDetail.comments.map((c) => (
                                                <div
                                                    key={c.id}
                                                    className="rounded border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-2 text-sm"
                                                >
                                                    <div className="flex items-center justify-between">
                                                        <span className="font-semibold">
                                                            {c.author}
                            ...
                            <div className="flex flex-wrap items-center gap-3 justify-center">
                                <div className="px-4 py-2 rounded-lg bg-[var(--surface)] shadow border border-[var(--border)]">
                                    ...
                                    <div className="text-2xl font-mono">
                                        {formatMs(elapsedMs)} s
                                    </div>
                                </div>
                                <div className="px-4 py-2 rounded-lg bg-[var(--surface)] shadow border border-[var(--border)]">
                                    <div className="text-xs opacity-70">剩餘雷</div>
                                    <div className="text-2xl font-mono">
                                        {remainingMines(boardForView)}
                                    </div>
                                </div>
                                <div className="px-4 py-2 rounded-lg bg-[var(--surface)] shadow border border-[var(--border)]">
                                    ...
                                    <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow p-4 space-y-3">
                                        ...
                                        {!vsMatch ? (
                                            ...
                                            <div className="space-y-3 text-sm">
                                                ...
                                                <p>
                                                    棋盤：{vsMatch.board.width}x
                                                    {vsMatch.board.height}，雷{' '}
                                                    {vsMatch.board.mines}
                                                </p>
                                                <p>
                                                    倒數：
                                        ...
                                        {recentError ? (
                                            ...
                                            <ol className="space-y-2 text-sm">
                                                {recentMatches.map((m) => (
                                                    <li
                                                        key={m.match_id}
                                                        className="border border-[var(--border)] rounded-lg px-3 py-2 bg-[var(--surface-strong)]"
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <span className="font-semibold">
                                                                #{m.match_id}
                    ...
                    {mode === 'versus' && vsMatch && vsState?.status === 'finished' && (
                        <section className="space-y-4">
                            <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow p-4 space-y-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {vsState.players.map((p) => (
                                        <button
                                            key={p.id}
                                            onClick={() =>
                                                setSelectedResultPlayerId(p.id)
                                            }
                                        ...
                                        >
                                            {p.name} (
                                            {renderResult(p.result, vsState.status)})
                                        </button>
                                    ))}
                                </div>
                                ...
                                {(() => {
                                    const p =
                                        vsState.players.find(
                                            (pl) => pl.id === selectedResultPlayerId
                                        ) ?? vsState.players[0];
                                    const snap = p
                                        ? getProgressBoard(p.progress ?? null)
                                        : null;
                                    if (!p)
                                        return (
                                            <p className="text-sm opacity-70">
